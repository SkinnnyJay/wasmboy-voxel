...
- [x] - S2-018 (cross-platform-shell): Remove shell-quoting assumptions in npm script wrappers for paths containing spaces/special characters. _(Normalized PATH override handling in automation subprocess wrappers/tests to avoid colon-only assumptions, added platform-aware PATH normalization hooks, and validated through full automation script suite.)_
- [x] - S2-019 (cross-platform-case): Add case-collision guardrails for generated artifact classification on macOS/Windows case-insensitive filesystems. _(Normalized generated-artifact policy checks to case-insensitive matching, added case-collision de-duplication in staged artifact blocking, and extended artifact policy/parity/guard tests to validate mixed-case + case-variant paths deterministically.)_
- [x] - S2-020 (cross-platform-tmpdir): Add long-path and reserved-device-name stress tests for temp fixture creation behavior on Windows. _(Added long nested temp-path stress coverage for `writeFakeExecutable`, added Windows-reserved temp-path segment validation in fixture creation, and introduced a Windows-gated reserved-device-path regression test while revalidating full automation suites.)_
- [x] - S2-021 (nutjs-baseline): Stand up NutJS-based cross-platform UI smoke harness for debugger flows (Linux/macOS/Windows). _(Added `scripts/nutjs-ui-smoke.mjs` baseline harness with CLI flags, module-load + host-capability checks, strict/fail-fast support, and JSON summaries; added dedicated harness contract tests and wired `npm run test:ui:smoke` into cross-platform CI smoke lanes with lane-specific logs.)_
- [x] - S2-022 (nutjs-input-map): Normalize NutJS keyboard scan-code mappings across OS layouts to remove flaky shortcut/input behavior. _(Added `scripts/nutjs-keyboard-layout-map.mjs` with platform-normalized shortcut scan-code plans + NutJS key-name translation, integrated default shortcut metadata into the NutJS smoke harness, and added dedicated keyboard-map + harness integration contract tests.)_
- [x] - S2-023 (nutjs-display-scale): Add DPI-aware coordinate transforms for NutJS pointer actions on Windows/macOS high-DPI displays. _(Added `scripts/nutjs-display-scale.mjs` with platform-specific DPI scale override handling (`NUTJS_WINDOWS_DPI_SCALE`, `NUTJS_MACOS_DPI_SCALE`, fallback `NUTJS_DPI_SCALE`) plus rounded pointer coordinate transforms, integrated pointer-transform metadata into the NutJS smoke harness, and added dedicated display-scale contract tests.)_
- [x] - S2-024 (nutjs-linux-display): Add X11/Wayland detection and stable fallback strategy for NutJS on Linux runners. _(Added `scripts/nutjs-linux-display.mjs` backend strategy resolution (auto/x11/wayland overrides), explicit Wayland enable gating, and deterministic Xvfb/X11 fallback recommendations; wired Linux strategy metadata into NutJS smoke capability checks and covered behavior with dedicated Linux-display + harness tests.)_
- [x] - S2-025 (nutjs-permissions): Add proactive accessibility permission checks/retry hints for NutJS on macOS. _(Added `scripts/nutjs-macos-permissions.mjs` with explicit macOS accessibility trust-state checks, deterministic retry-hint guidance, and strict env flag validation; integrated permission-state metadata/reason propagation into NutJS smoke capability checks and added dedicated macOS permission + harness tests.)_
- [x] - S2-026 (nutjs-image-match): Calibrate NutJS image matching thresholds per OS/compositor to reduce false negatives. _(Added `scripts/nutjs-image-thresholds.mjs` with per-platform/per-backend threshold profiles (Linux X11/Wayland, macOS, Windows), deterministic override precedence, and strict threshold validation; integrated resolved threshold metadata into NutJS smoke defaults and added dedicated threshold calibration tests.)_
- [x] - S2-027 (nutjs-memory): Add leak guard for repeated NutJS screenshots/template matching in long UI test runs. _(Added `scripts/nutjs-memory-guard.mjs` bounded snapshot tracker with count/byte caps, eviction accounting, and explicit dispose semantics to prevent retained-buffer growth; integrated guard telemetry into default NutJS smoke metadata and added contract coverage for tracking, eviction, and disposal behavior.)_
- [x] - S2-028 (nutjs-process-cleanup): Ensure NutJS child processes/resources are always disposed on failures/timeouts. _(Added `scripts/nutjs-resource-cleanup.mjs` with timeout-bounded action execution and guaranteed `dispose()` finalization on success/error/timeout, integrated cleanup-session support into the NutJS smoke harness (`createSession`, `timeoutMs`), and added cleanup contract coverage plus timeout-disposal verification through harness tests.)_
- [x] - S2-029 (nutjs-artifacts): Standardize deterministic screenshot/video artifact naming and retention for NutJS triage outputs. _(Added `scripts/nutjs-artifacts.mjs` deterministic artifact naming (`png/webm/jsonl`) with token sanitization + timestamp override support, plus retention-window selection for pruning older files; integrated artifact sample/retention metadata into NutJS smoke defaults and added contract tests for naming, retention, and input validation.)_
- [x] - S2-030 (nutjs-simplification): Create a small shared NutJS action DSL to simplify test flows and remove duplicated wait/retry boilerplate. _(Added `scripts/nutjs-action-dsl.mjs` with normalized step contracts, built-in wait/retry handling, deterministic timelines, and clear failure envelopes; refactored NutJS smoke default metadata collection to execute through the shared DSL and added dedicated DSL contract tests + harness timeline assertions.)_
...
- [x] - Phase 11: Convert worker entry points to `.ts`; type message schemas (align with Phase 0 docs). _(Added TypeScript worker entrypoint wrappers for graphics/audio/controller/memory/wasmboy workers and switched Rollup worker inputs to `.ts`, plus introduced shared worker message-envelope schema helpers (`lib/worker/message-schema.js` + `.d.ts`) and integrated envelope guards across worker message handlers.)_
- [x] - Phase 11: Type all snapshot and memory paths used by voxel-wrapper (use contracts from packages/api where applicable). _(Aligned voxel-wrapper snapshot/memory contract types with `packages/api` contract payloads using type imports for `MemorySection`, `PpuSnapshot`, and `Registers`, while preserving Uint8Array runtime surfaces for snapshot/memory reads.)_
- [x] - Phase 11: Convert `lib/headless/` to `.ts`; align with `wasm-fork.d.ts` headless module and `WasmBoyHeadless` / `loadMainThreadWasm` types. _(Added TypeScript headless entry wrappers (`*.entry.ts`) for `WasmBoyHeadless`, main-thread core loaders, and adapter surfaces; expanded headless declaration files and aligned `wasm-fork.d.ts` with explicit `WasmBoyHeadless` button/core-load types.)_
- [x] - Phase 11: Remove all `any` and unsafe type assertions in `lib/`. _(Current TypeScript migration surfaces in `lib/**/*.ts` and declaration files avoid `any` and unsafe `as` assertions; unresolved dynamic surfaces are explicitly tracked as `unknown` in the intentional narrowing audit.)_
- [x] - Phase 11: Add shared types/constants in `lib/` or from `packages/shared` (no magic numbers for non-hardware). _(Added shared headless constants module (`lib/headless/constants.ts` + `.d.ts`) and refactored `WasmBoyHeadless` to consume named constants for frame sizing, ROM/RAM limits, address bounds, byte masks, schema IDs, and button mappings.)_
- [x] - Phase 11: Ensure `voxel-wrapper.ts` and `index.ts` consume typed lib; no new `any`. _(Updated voxel wrapper to consume typed fork/API contract surfaces (`wasm-fork` + `packages/api` contract types) and kept root `index.ts` as typed re-export surface with no additional `any` introduction.)_
...
- [x] - Phase 11: Document any remaining intentional `unknown` or narrowings. _(Added `docs/migration/lib-ts-intentional-unknowns-2026-02-14.md` to track current intentional `unknown` usage buckets in migrated lib declarations and the planned narrowing strategy.)_
