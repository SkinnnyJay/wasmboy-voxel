...
test('resolveStrictPositiveIntegerEnv rejects bigint option names', () => {
  assert.throws(
    () =>
      resolveStrictPositiveIntegerEnv({
        name: 42n,
...
test('resolveStrictPositiveIntegerEnv rejects negative zero default values', () => {
  assert.throws(
    () =>
      resolveStrictPositiveIntegerEnv({
        name: 'TEST_TIMEOUT',
...
test('resolveTimeoutFromCliAndEnv rejects symbol top-level options', () => {
...
test('resolveTimeoutFromCliAndEnv rejects symbol env option values', () => {
  assert.throws(
    () =>
      resolveTimeoutFromCliAndEnv({
        defaultValue: 120000,
...
test('resolveTimeoutFromCliAndEnv rejects symbol cli option values', () => {
  assert.throws(
    () =>
      resolveTimeoutFromCliAndEnv({
        defaultValue: 120000,
...
test('resolveTimeoutFromCliAndEnv rejects negative zero default timeout values', () => {
  assert.throws(
    () =>
      resolveTimeoutFromCliAndEnv({
        defaultValue: -0,
...
test('resolveTimeoutFromCliAndEnv rejects bigint environment option names', () => {
  assert.throws(
    () =>
      resolveTimeoutFromCliAndEnv({
        defaultValue: 120000,
        env: { name: 42n, rawValue: undefined },
        cli: { name: '--test-timeout', rawValue: undefined },
      }),
    /Invalid timeout option name: 42/u,
  );
...
test('resolveTimeoutFromCliAndEnv rejects bigint cli option names', () => {
  assert.throws(
    () =>
      resolveTimeoutFromCliAndEnv({
        defaultValue: 120000,
        env: { name: 'TEST_TIMEOUT_ENV', rawValue: undefined },
        cli: { name: 42n, rawValue: undefined },
      }),
    /Invalid timeout option name: 42/u,
  );
});
