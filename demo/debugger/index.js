...
// and automation work. ResponsiveGamepad still receives the key for START (Enter) and
// SPECIAL (Space = play/pause hotkey). Mappings: START = Enter, SELECT = Shift,
// SPECIAL = Space (debugger play/pause). See help and iframe About for full controls.
const preventButtonActivationForGameKeys = event => {
  const el = document.activeElement;
  if (el && el.tagName === 'BUTTON' && (event.code === 'Space' || event.code === 'Enter')) {
    event.preventDefault();
  }
};
window.addEventListener('keydown', preventButtonActivationForGameKeys, true);
