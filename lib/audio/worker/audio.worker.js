...
const getAudioChannelBuffersFromBuffer = (audioBuffer, numberOfSamples) => {
  ...
  const numberOfSamplesForStereo = numberOfSamples * 2;
  for (let i = 0; i < numberOfSamples; ++i) {
    leftChannelBuffer[i] = getUnsignedAudioSampleAsFloat(audioBuffer[i * 2]);
    rightChannelBuffer[i] = getUnsignedAudioSampleAsFloat(audioBuffer[i * 2 + 1]);
  }
  return {
