...
import { parsePpuSnapshotBuffer as parsePpuSnapshotBufferImpl } from '../ppuSnapshotParse';
import {
  PPU_SNAPSHOT_TOTAL,
...
export const getWasmMemorySection = async (start, end) => {
  ...
  if (WasmBoyLib.wasmByteMemory) {
    const e = end != null ? end : WasmBoyLib.wasmByteMemory.length;
    return new Uint8Array(WasmBoyLib.wasmByteMemory.slice(start, e));
  }
  const event = await WasmBoyLib.worker.postMessage({
...
export const getWasmConstant = async constantKey => {
  ...
  if (WasmBoyLib.wasmInstance && WasmBoyLib.wasmInstance.exports[constantKey] !== undefined) {
    return WasmBoyLib.wasmInstance.exports[constantKey].valueOf();
  }
  const event = await WasmBoyLib.worker.postMessage({
...
export const getPpuSnapshotBuffer = async () => {
  ...
  if (WasmBoyLib.wasmInstance && WasmBoyLib.wasmByteMemory) {
    const mem = WasmBoyLib.wasmByteMemory;
    const base = WasmBoyLib.wasmInstance.exports.GAMEBOY_INTERNAL_MEMORY_LOCATION.valueOf();
...
export const parsePpuSnapshotBuffer = parsePpuSnapshotBufferImpl;
