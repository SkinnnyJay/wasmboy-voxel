...
import { loadMainThreadWasm } from '../headless/mainThreadCore.wasm.js';
import { createMainThreadWorker } from '../headless/mainThreadAdapter.js';

/*ROLLUP_REPLACE_NODE
const { MessageChannel } = require('worker_threads');
ROLLUP_REPLACE_NODE*/

export const instantiateWorkers = async (options = {}) => {
  if (options.headless && options.mainThread) {
    const wasm = await loadMainThreadWasm();
    const { worker } = createMainThreadWorker(wasm);
    return { worker, wasmInstance: wasm.instance, wasmByteMemory: wasm.byteMemory };
  }

  // Replaced at build time: WASM -> wasmboyLibWasmWorkerUrl, TS -> wasmboyLibTsWorkerUrl
  const libWorkerUrl = __LIB_WORKER_URL__;
