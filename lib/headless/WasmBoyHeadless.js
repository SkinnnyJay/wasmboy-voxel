...
import {
  HEADLESS_BYTE_MASK,
  HEADLESS_BUTTON_INDEX,
  HEADLESS_CARTRIDGE_RAM_MAX_BYTES,
  HEADLESS_FRAME_BUFFER_RGBA_SIZE,
  HEADLESS_GAMEBOY_ADDRESS_MAX,
  HEADLESS_GAMEBOY_ADDRESS_MIN,
  HEADLESS_ROM_MAX_BYTES,
...
export class WasmBoyHeadless {
  ...
  async loadROM(rom, options = {}) {
    ...
    const maxRom = Math.min(romBytes.length, HEADLESS_ROM_MAX_BYTES);
  ...
  getFrameBuffer() {
    if (!this.byteMemory || !this._frameSize) {
      return new Uint8ClampedArray(HEADLESS_FRAME_BUFFER_RGBA_SIZE);
    }
   ...
   */
  readMemory(address) {
    if (!this.byteMemory || address < HEADLESS_GAMEBOY_ADDRESS_MIN || address > HEADLESS_GAMEBOY_ADDRESS_MAX) return 0;
  ...
  writeMemory(address, value) {
    if (!this.byteMemory || address < HEADLESS_GAMEBOY_ADDRESS_MIN || address > HEADLESS_GAMEBOY_ADDRESS_MAX) return;
    const offset = this._debugGameboyMemoryLocation + address;
    if (offset < this.byteMemory.length) {
      this.byteMemory[offset] = value & HEADLESS_BYTE_MASK;
    }
  }
  ...
  setButton(button, pressed) {
    const idx = HEADLESS_BUTTON_INDEX[button];
    if (idx !== undefined) {
  ...
  saveState() {
    ...
    return {
      ...
      schema: HEADLESS_SCHEMA_VERSION,
    };
  }
  ...
  loadState(state) {
    ...
    if (state.schema === HEADLESS_SCHEMA_VERSION && state.byteMemory) {
    ...
    if (wm) {
      ...
      set(wm.cartridgeRam, this._cartridgeRamLocation, HEADLESS_CARTRIDGE_RAM_MAX_BYTES);
    }
  }
