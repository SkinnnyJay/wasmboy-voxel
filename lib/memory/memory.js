...
class WasmBoyMemoryService {
      ...
      switch (eventData.message.type) {
        case WORKER_MESSAGE_TYPE.UPDATED: {
          ...
          if (memoryTypes.includes(MEMORY_TYPE.CARTRIDGE_RAM)) {
            const buf = eventData.message[MEMORY_TYPE.CARTRIDGE_RAM];
            if (this.cartridgeRam && this.cartridgeRam.byteLength === buf.byteLength) {
              this.cartridgeRam.set(new Uint8Array(buf));
            } else {
              this.cartridgeRam = new Uint8Array(buf);
            }
          }
          if (memoryTypes.includes(MEMORY_TYPE.GAMEBOY_MEMORY)) {
            const buf = eventData.message[MEMORY_TYPE.GAMEBOY_MEMORY];
            if (this.gameboyMemory && this.gameboyMemory.byteLength === buf.byteLength) {
              this.gameboyMemory.set(new Uint8Array(buf));
            } else {
              this.gameboyMemory = new Uint8Array(buf);
            }
          }
          if (memoryTypes.includes(MEMORY_TYPE.PALETTE_MEMORY)) {
            const buf = eventData.message[MEMORY_TYPE.PALETTE_MEMORY];
            if (this.paletteMemory && this.paletteMemory.byteLength === buf.byteLength) {
              this.paletteMemory.set(new Uint8Array(buf));
            } else {
              this.paletteMemory = new Uint8Array(buf);
            }
          }
          if (memoryTypes.includes(MEMORY_TYPE.INTERNAL_STATE)) {
            const buf = eventData.message[MEMORY_TYPE.INTERNAL_STATE];
            if (this.internalState && this.internalState.byteLength === buf.byteLength) {
              this.internalState.set(new Uint8Array(buf));
            } else {
              this.internalState = new Uint8Array(buf);
            }
          }
  ...
  async _fetchMemory(memoryTypes) {
    if (!this.worker) {
      throw new Error('WasmBoy worker is not initialized');
    }
  ...
  async getGameboyMemory() {
    const message = await this._fetchMemory([MEMORY_TYPE.GAMEBOY_MEMORY]);
    const buffer = message[MEMORY_TYPE.GAMEBOY_MEMORY];
    if (!buffer) {
      return new Uint8Array();
    }
    this.gameboyMemory = new Uint8Array(buffer);
    return new Uint8Array(this.gameboyMemory);
  }

  async getFullMemory() {
    return this.getGameboyMemory();
  }

  async readMemory(address, length) {
    if (address < 0 || length < 0 || address + length > GAMEBOY_MEMORY_SIZE) {
      throw new Error('readMemory address out of bounds');
    }
    const memory = await this.getGameboyMemory();
    return memory.slice(address, address + length);
  }

  async getCartridgeRam() {
    const message = await this._fetchMemory([MEMORY_TYPE.CARTRIDGE_RAM]);
    const buffer = message[MEMORY_TYPE.CARTRIDGE_RAM];
    if (!buffer) {
      return new Uint8Array();
    }
    this.cartridgeRam = new Uint8Array(buffer);
    return new Uint8Array(this.cartridgeRam);
  }

  async getWRAM() {
    const memory = await this.getGameboyMemory();
    return memory.slice(WORK_RAM_START, WORK_RAM_START + WORK_RAM_SIZE);
  }
  ...
  async setWRAM(payload) {
    if (!(payload instanceof Uint8Array)) {
      throw new Error('setWRAM expects a Uint8Array payload');
    }
    if (payload.length !== WORK_RAM_SIZE) {
      throw new Error('setWRAM payload size mismatch');
    }
    return this.writeRAM(WORK_RAM_START, payload);
  }

  async setWorkRAM(payload) {
    return this.setWRAM(payload);
  }

  async writeRAM(address, payload) {
    if (!(payload instanceof Uint8Array)) {
      throw new Error('writeRAM expects a Uint8Array payload');
    }
    if (address < 0 || address + payload.length > GAMEBOY_MEMORY_SIZE) {
      throw new Error('writeRAM address out of bounds');
    }
    const memory = await this.getGameboyMemory();
    memory.set(payload, address);

    const clone = memory.buffer.slice(0);
    const workerMemoryObject = {};
    workerMemoryObject[MEMORY_TYPE.GAMEBOY_MEMORY] = clone;
    ...
    this.gameboyMemory = new Uint8Array(memory);
    return true;
  }
