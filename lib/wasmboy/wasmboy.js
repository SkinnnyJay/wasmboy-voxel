...
class WasmBoyLibService {
  ...
  constructor() {
    ...
    this.renderId = false;
    this.mainThreadUpdateId = undefined;
  ...
  config(wasmBoyOptions, canvasElement) {
    const configTask = async () => {
      ...
      // Headless: skip canvas; non-headless set canvas
  ...
  /**
   * Promise that resolves when the emulator is ready (workers and WASM initialized).
   * Rejects after timeoutMs (default 15000) with { code: 'TIMEOUT', message }.
    ...
    return new Promise((resolve, reject) => {
      ...
      const id = setInterval(() => {
        ...
        if (Date.now() - start >= timeoutMs) {
          clearInterval(id);
          reject(Object.assign(new Error('whenReady timed out'), { code: 'TIMEOUT' }));
        }
      }, 50);
    });
  }
  ...
  /**
   * Promise that resolves when the emulator is playing. Rejects after timeoutMs (default 5000).
   * @param {number} [timeoutMs=5000]
   * @returns {Promise<void>}
   */
  whenPlaying(timeoutMs = 5000) {
    if (!this.paused) {
      return Promise.resolve();
    }
    return new Promise((resolve, reject) => {
      ...
      const id = setInterval(() => {
        ...
        if (Date.now() - start >= timeoutMs) {
          clearInterval(id);
          reject(Object.assign(new Error('whenPlaying timed out'), { code: 'TIMEOUT' }));
        }
      }, 50);
    });
  }
  ...
  /**
   * Promise that resolves when the emulator is paused. Rejects after timeoutMs (default 5000).
   * @param {number} [timeoutMs=5000]
   * @returns {Promise<void>}
   */
  whenPaused(timeoutMs = 5000) {
    if (this.paused) {
      return Promise.resolve();
    }
    return new Promise((resolve, reject) => {
      ...
      const id = setInterval(() => {
        ...
        if (Date.now() - start >= timeoutMs) {
          clearInterval(id);
          reject(Object.assign(new Error('whenPaused timed out'), { code: 'TIMEOUT' }));
        }
      }, 50);
    });
  }
  ...
  play() {
    const playTask = async () => {
      ...
      if (this.wasmInstance && this.wasmByteMemory) {
        const intervalRate = Math.max(1, Math.floor(1000 / this.options.gameboyFrameRate));
        this.mainThreadUpdateId = setInterval(() => {
          ...
          this.worker
            ...
            .then(() => {
              if (!this.wasmInstance || !this.wasmByteMemory || this.paused) return;
              const frameLoc = this.wasmInstance.exports.FRAME_LOCATION.valueOf();
              const frameSize = this.wasmInstance.exports.FRAME_SIZE.valueOf();
              const slice = this.wasmByteMemory.slice(frameLoc, frameLoc + frameSize);
              const imageData = getImageDataFromGraphicsFrameBuffer(slice);
              if (this.options.updateGraphicsCallback && imageData) {
                this.options.updateGraphicsCallback(imageData);
              }
            });
        }, intervalRate);
      } else if (!this.renderId && !this.options.headless) {
        this.renderId = raf(() => {
          render.call(this);
        });
      }
  ...
  pause() {
    const pauseTask = async () => {
      ...
      this.renderId = false;
      if (this.mainThreadUpdateId) {
        clearInterval(this.mainThreadUpdateId);
        this.mainThreadUpdateId = undefined;
      }
  ...
  _instantiateWorkers() {
    const instantiateWorkersTask = async () => {
      ...
      const result = await instantiateWorkers(this.options);
      if (result.wasmInstance) {
        this.worker = result.worker;
        this.wasmInstance = result.wasmInstance;
        this.wasmByteMemory = result.wasmByteMemory;
      } else {
        this.worker = result;
      }
