...
export function libWorkerOnMessage(event) {
  ...
  if (messageRequests[eventData.message.type]) {
    const waiters = messageRequests[eventData.message.type];
    waiters.forEach(request => request(eventData.message));
    delete messageRequests[eventData.message.type];
  }
  ...
  switch (eventData.message.type) {
    ...
    case WORKER_MESSAGE_TYPE.BREAKPOINT: {
      const breakpointPayload = eventData.message.breakpoint || null;
      const breakpointTask = async () => {
        if (this.options.breakpointPause !== false) {
          await this.pause();
        } else if (this.worker && this.worker.postMessageIgnoreResponse) {
          this.worker.postMessageIgnoreResponse({
            type: WORKER_MESSAGE_TYPE.CONTINUE_AFTER_BREAKPOINT,
          });
        }
        if (this.options.breakpointCallback) {
          this.options.breakpointCallback(breakpointPayload);
        }
