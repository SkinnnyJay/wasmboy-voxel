    ...
    switch (eventData.message.type) {
      ...
      case WORKER_MESSAGE_TYPE.GET_PPU_SNAPSHOT: {
        if (!libWorker.wasmInstance || !libWorker.wasmByteMemory) {
          postMessage(
            getSmartWorkerMessage(
              { type: WORKER_MESSAGE_TYPE.GET_PPU_SNAPSHOT, response: null },
              eventData.messageId
            )
          );
          return;
        ...
        for (let i = 0; i < PPU_SNAPSHOT_MAP_LEN; i++)
          out[PPU_SNAPSHOT_TILE_LEN + PPU_SNAPSHOT_MAP_LEN + i] = mem[map1Start + i];
        for (let i = 0; i < PPU_SNAPSHOT_OAM_LEN; i++)
          out[PPU_SNAPSHOT_TILE_LEN + PPU_SNAPSHOT_MAP_LEN * 2 + i] = mem[oamStart + i];
