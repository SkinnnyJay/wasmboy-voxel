...
import { buildInvalidWorkerEnvelopeError, hasWorkerMessageType } from '../../worker/message-schema';
import {
...
import { update, scheduleNextUpdate } from './update';
...
libWorker = {
  ...
  messageHandler: event => {
    ...
    if (!hasWorkerMessageType(eventData)) {
      postMessage(getSmartWorkerMessage(buildInvalidWorkerEnvelopeError(''), eventData && eventData.messageId));
      return;
    }
    ...
    switch (eventData.message.type) {
      ...
      case WORKER_MESSAGE_TYPE.SET_WASM_MEMORY_SECTION: {
        ...
        let start = 0;
        if (typeof eventData.message.start === 'number') {
          start = eventData.message.start;
        }
        ...
        libWorker.wasmByteMemory.set(bytes, start);
        postMessage(
          getSmartWorkerMessage(
            {
      ...
      case WORKER_MESSAGE_TYPE.GET_WASM_CONSTANT: {
        ...
        let response;
        try {
          if (!libWorker.wasmInstance || !libWorker.wasmInstance.exports[name]) {
            postMessage(
              getSmartWorkerMessage(
                { type: WORKER_MESSAGE_TYPE.GET_WASM_CONSTANT, error: true, code: 'CONSTANT_NOT_FOUND', constant: name },
                eventData.messageId,
              ),
            );
            return;
          }
          response = libWorker.wasmInstance.exports[name].valueOf();
        } catch (e) {
          postMessage(
            getSmartWorkerMessage(
              { type: WORKER_MESSAGE_TYPE.GET_WASM_CONSTANT, error: true, code: 'CONSTANT_READ_ERROR', constant: name },
              eventData.messageId,
            ),
          );
          return;
      ...
      case WORKER_MESSAGE_TYPE.GET_PPU_SNAPSHOT: {
        if (!libWorker.wasmInstance || !libWorker.wasmByteMemory) {
          postMessage(getSmartWorkerMessage({ type: WORKER_MESSAGE_TYPE.GET_PPU_SNAPSHOT, response: null }, eventData.messageId));
          return;
        }
        const mem = libWorker.wasmByteMemory;
        const base = libWorker.wasmInstance.exports.GAMEBOY_INTERNAL_MEMORY_LOCATION.valueOf();
          ...
          getSmartWorkerMessage(
            {
              ...
              messageType: eventData.message && eventData.message.type,
            },
            eventData.messageId,
          ),
        );
