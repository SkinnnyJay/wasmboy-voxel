...
export function scheduleNextUpdate(libWorker) {
...
export function update(libWorker, passedIntervalRate) {
  ...
  if (libWorker.frameCounter === undefined) {
    libWorker.frameCounter = 0;
  }
  ...
  const executePromise = new Promise(resolve => {
    if (libWorker.hasPendingJoypadState && libWorker.pendingJoypadState) {
      libWorker.wasmInstance.exports.setJoypadState.apply(libWorker, libWorker.pendingJoypadState);
      libWorker.hasPendingJoypadState = false;
    }
  ...
  executePromise.then(response => {
    ...
    if (response >= 0) {
      ...
      // Transfer Memory only when needed (save states, debugger, or syncMemoryEveryFrame)
      if (libWorker.options.syncMemoryEveryFrame) {
        const memoryObject = {
          type: WORKER_MESSAGE_TYPE.UPDATED
        };
      ...
      if (response === 2) {
        const exports = libWorker.wasmInstance.exports;
        const getAddress = exports.getLastBreakpointAddress;
        const getAccess = exports.getLastBreakpointAccess;
        const address = typeof getAddress === 'function' ? getAddress() : -1;
        const accessCode = typeof getAccess === 'function' ? getAccess() : 0;
        const access = accessCode === 1 ? 'write' : 'read';
        const id = access === 'read' ? 'bp-read' : 'bp-write';
        postMessage(
          getSmartWorkerMessage({
            ...
            breakpoint: {
              ...
              address,
              access,
              firedAtMs: typeof performance !== 'undefined' && performance.now ? Math.floor(performance.now()) : Date.now()
            }
          })
        );
      } else {
